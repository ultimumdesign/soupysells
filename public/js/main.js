const soupysells = angular.module("soupysells", [
    "ngRoute",
    "ngTouch",
    "chart.js"
]);
...
soupysells.service("listService", function($http) {
    ...
    this.post = function(string, type) {
        ...
        return $http
            .post(url, JSON.stringify(dataObj))
...
soupysells.service("metricService", function($http) {
    ...
    this.getTaxes = function(dateMonth, dateYear) {
        let url =
            "/api/metric/taxreport?dateMonth=" +
            dateMonth +
            "&dateYear=" +
            dateYear;
        return $http.get(url).then(function(response) {
...
/** CONTROLLER **/
soupysells.controller("saleController", function(
    $scope,
    $window,
    $route,
    listService,
    ...
    $scope.invItemToSaleServiceUpdate = function(data) {
        itemToSaleService.update(data).then(function(data) {
            if (data.error) {
                //do something
            } else {
    ...
    $scope.toggleEditModal = function(element) {
        if (element) {
            ...
            $scope.edited.saledate = new Date($scope.edited.saledate);
    ...
    $scope.invMetricServiceGetTax = function() {
        ...
        metricService.getTaxes(dateMonth, dateYear).then(function(data) {
            if (data.error) {
                //do something
            } else {
                //assign something, do something
    ...
    $scope.invMetricServiceGet = function() {
        if ($scope.reportOption.typeFilter == "Sales") {
            metricService
                .getSalesReport($scope.reportOption.dateFilter)
                .then(function(data) {
                    if (data.error) {
                        //do something
                    } else {
            ...
            metricService
                .getItemsReport($scope.reportOption.dateFilter)
                .then(function(data) {
                    if (data.error) {
                        //do something
                    } else {
            ...
            if (data.error) {
                ...
                listService.getSellingplat().then(function(data) {
                    if (data.error) {
                        ...
                        $scope.lists.sellingplat = data;
                        let setObj = findObjectByKey(
                            $scope.lists.sellingplat,
                            "ID",
                            postID
                        );
    ...
    $scope.checkAddExist = function(testList) {
        ...
        angular.forEach(testList, function(value, key) {
            if ($scope.addSellingplat) {
                if (
                    $scope.addSellingplat.name.toUpperCase() ==
...
/** CONTROLLER **/
soupysells.controller("itemController", function(
    $scope,
    $window,
    $route,
    listService,
        ...
        if (type == "purch") {
            angular.forEach(testList, function(value, key) {
                if ($scope.addpurchloc) {
                    if (
                        $scope.addpurchloc.name.toUpperCase() ==
                        value.name.toUpperCase()
                    ) {
                        doesExist = true;
                        return doesExist;
